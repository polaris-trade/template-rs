...
samply BIN:
    cargo --config @.cargo/profiling.toml build --profile profiling --bin {{BIN}}
    samply record ./target/profiling/{{BIN}}

# Generate a flamegraph via cargo-flamegraph. Usage: just flame <bin-name>
flame BIN:
    cargo --config @.cargo/flamegraph.toml flamegraph --profile flamegraph --bin {{BIN}}
...
msrv VERSION:
    sed -i.bak -E 's/^(\s*channel\s*=\s*)"[^"]+"/\1"{{VERSION}}"/' rust-toolchain.toml
    sed -i.bak -E 's/^(\s*rust-version\s*=\s*)"[^"]+"/\1"{{VERSION}}"/' Cargo.toml
    rm -f rust-toolchain.toml.bak Cargo.toml.bak
    @echo "MSRV updated to {{VERSION}}. Commit the change and CI will re-resolve the matrix."
